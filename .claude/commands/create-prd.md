...
- Monorepo workspace structure (explicitly map `/apps` and `/packages` or specific backend/frontend directories)
- Shared dependencies strategy (how types, utilities, or configurations are shared across the stack)
...
**8. Technology Stack**
- Monorepo Orchestration (e.g., NPM Workspaces, Turborepo, concurrently)
...
- MUST use "Vertical Slicing": Each phase must deliver a complete, testable feature traversing the entire stack (Database -> Backend -> Frontend)
- Each phase includes: Goal, Deliverables (✅ checkboxes), Validation criteria
- Validation criteria MUST include end-to-end testing instructions bridging the frontend and backend
...
- For full-stack applications, strictly enforce monorepo boundaries. Never allow frontend code to directly import backend execution logic, and ensure cross-stack communication relies strictly on defined API contracts.
